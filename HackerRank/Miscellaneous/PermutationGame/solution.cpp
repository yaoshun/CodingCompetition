...
// Only two outcomes, we can do it easier with 0 as loss for the current player 
// and 1 as win condition for the current player..
int solve(int node) {
    if (tree[node] != -1) {
        return tree[node];
    } else {
        // mark it as losing position for the current player.
        tree[node] = 0;
    ...
    while (T--) {
        ...
        for (int i = 0; i < n; i++) {
            ...
            for (int j = 0; j < offset; j++) {
                // this is ending position with an increaing array.
                // therefore a losing position for the current player.
